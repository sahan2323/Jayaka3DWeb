  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center gap-4 rounded-3xl border border-cocoa/10 bg-white px-8 py-16 text-center shadow-sm">
        ...
        <button type="button" onClick={() => { setSubmitted(false); setServerError(null); }}
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="grid gap-6 sm:grid-cols-2">
      ...
      <Field label="Name" error={errors.name?.message}>
        <input type="text" autoComplete="name" {...register("name")} className={inputClass(!!errors.name)} placeholder="Jane Cooper" />
      </Field>
      <Field label="Company" error={errors.company?.message}>
        <input type="text" autoComplete="organization" {...register("company")} className={inputClass(!!errors.company)} placeholder="Your company" />
      </Field>
      <Field label="Email" error={errors.email?.message}>
        <input type="email" autoComplete="email" {...register("email")} className={inputClass(!!errors.email)} placeholder="you@company.com" />
      </Field>
      <Field label="Country" error={errors.country?.message}>
        <input type="text" autoComplete="country-name" {...register("country")} className={inputClass(!!errors.country)} placeholder="Country" />
      </Field>
      <Field label="Product Interest" error={errors.productInterest?.message} full>
        <select {...register("productInterest")} defaultValue="" className={inputClass(!!errors.productInterest)}>
          <option value="" disabled>Select a product category</option>
          {productOptions.map((c) => (<option key={c.id} value={c.label}>{c.label}</option>))}
        </select>
      </Field>
      <Field label="Message" error={errors.message?.message} full>
        <textarea rows={5} {...register("message")} className={inputClass(!!errors.message)}
